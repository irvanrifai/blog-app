...
@section('container')
    <div class="w-full max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
        ...
        <div class="flex flex-col items-center pb-10">
            <img class="mb-3 w-24 h-24 rounded-full shadow-lg" src="{{ asset('storage/' . auth()->user()->photo) }}"
                alt="Bonnie image">
            <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-white">{{ auth()->user()->name }}</h5>
            <span class="text-sm text-gray-500 pb-4 dark:text-gray-400">{{ auth()->user()->username }}</span>
            <div class="flex mt-4 space-x-3 md:mt-8">
                <div class="flex justify-center mb-4">
                    <a href="{{ auth()->user()->fb_account ? auth()->user()->fb_account : '#!' }}"
